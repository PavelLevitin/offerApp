...
describe('Fresh Onboarding — Step 7: Registration form validations', () => {
  ...
  it('should show 6 validation errors on empty form submit', async () => {
    ...
    const nameErrors1     = await $$('//*[contains(@content-desc, "שם קצת קצר, לא?") or contains(@text, "שם קצת קצר, לא?")]');
    const reminderErrors1 = await $$('//*[contains(@content-desc, "היי, שכחת אותי") or contains(@text, "היי, שכחת אותי")]');
    const emailErrors1    = await $$(`//*[contains(@content-desc, 'כתובת הדוא"ל אינה תקינה') or contains(@text, 'כתובת הדוא"ל אינה תקינה')]`);
    const termsErrors1    = await $$('//*[contains(@content-desc, "יש לאשר") or contains(@text, "יש לאשר")]');
    expect(nameErrors1.length + reminderErrors1.length + emailErrors1.length + termsErrors1.length).toBeGreaterThanOrEqual(6);
  });
  ...
  it('should show error for 1-character first name', async () => {
    ...
    await expect($('//*[contains(@content-desc, "שם קצת קצר, לא?") or contains(@text, "שם קצת קצר, לא?")]')).toBeDisplayed();
  ...
  it('should clear first name error for 2-character first name', async () => {
    ...
    const nameErrors2 = await $$('//*[contains(@content-desc, "שם קצת קצר, לא?") or contains(@text, "שם קצת קצר, לא?")]');
  ...
  it('should enforce 20-character limit on first name', async () => {
    await RegistrationPage1.fillFirstName('abcdefghijklmnopqrstu', false, false);
  ...
  it('should reduce errors to 5 after valid first name entry', async () => {
    ...
    await RegistrationPage1.tapContinue();
    const nameErrors3     = await $$('//*[contains(@content-desc, "שם קצת קצר, לא?") or contains(@text, "שם קצת קצר, לא?")]');
    const reminderErrors3 = await $$('//*[contains(@content-desc, "היי, שכחת אותי") or contains(@text, "היי, שכחת אותי")]');
    const emailErrors3    = await $$(`//*[contains(@content-desc, 'כתובת הדוא"ל אינה תקינה') or contains(@text, 'כתובת הדוא"ל אינה תקינה')]`);
    const termsErrors3    = await $$('//*[contains(@content-desc, "יש לאשר") or contains(@text, "יש לאשר")]');
    expect(nameErrors3.length + reminderErrors3.length + emailErrors3.length + termsErrors3.length).toBeGreaterThanOrEqual(5);
  });
  ...
  it('should clear last name error for 2-character last name', async () => {
    ...
    await RegistrationPage1.emailInput.click();
    await expect($('//*[contains(@content-desc, "שם קצת קצר, לא?") or contains(@text, "שם קצת קצר, לא?")]')).not.toBeDisplayed();
    await RegistrationPage1.lastNameInput.click();
    await driver.pause(300);
    await clearField('ab'.length + 2);
  });
  ...
  it('should clear gender error after selection', async () => {
    ...
    await driver.pause(500);
    const reminderErrors10 = await $$('//*[contains(@content-desc, "היי, שכחת אותי") or contains(@text, "היי, שכחת אותי")]');
  ...
  it('should show error for each invalid email format', async () => {
    ...
    if (failures.length > 0) throw new Error(`Email validation failures:\n${failures.join('\n')}`);
  });
